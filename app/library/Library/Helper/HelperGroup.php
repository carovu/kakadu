...
class HelperGroup {
    ...
    public static function getLearngroupsOfUser($user) {
        ...
        foreach($userKakadu->learngroups()->orderBy('name')->get() as $g) {
            $groups[] = array(
                'id'            => $g->id,
                'name'          => $g->name,
    ...
    public static function deleteFavoritesOfLearngroupMember($group, $user) {
        ...
        foreach($group->courses()->get() as $course) {
            ...
            if($check === false) {
                $catalog = $course->catalog()->first();
                $catalogIDs = HelperCourse::getSubCatalogIDsOfCatalog($catalog);
                DB::table('favorites')->whereIn('catalog_id', $catalogIDs)->delete();
            }
        }
