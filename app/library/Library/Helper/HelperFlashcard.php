...
class HelperFlashcard {
    ...
    public static function saveFlashcard($user, $question, $answer, $catalogs) {
        ...
        $query = static::getQuery($user_id, $catalogs);
        $result = $query->where('flashcards.offset', '>', 0)->get(array('flashcards.id as id'));
        if(count($result) > 0) {
            $flashcardIDs = array();
            ...
            DB::table('flashcards')
                ->whereIn('flashcards.id', $flashcardIDs)
